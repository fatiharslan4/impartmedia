...
var UserWhere = struct {
	...
	CreatedTS:        whereHelpertime_Time{field: "`user`.`created_ts`"},
	UpdatedTS:        whereHelpertime_Time{field: "`user`.`updated_ts`"},
...
var (
	userAllColumns            = []string{"impart_wealth_id", "authentication_id", "email", "screen_name", "created_ts", "updated_ts", "device_token", "aws_sns_app_arn", "admin"}
	userColumnsWithoutDefault = []string{"impart_wealth_id", "authentication_id", "email", "screen_name", "created_ts", "updated_ts", "device_token", "aws_sns_app_arn", "admin"}
...
func FindUser(ctx context.Context, exec boil.ContextExecutor, impartWealthID string, selectCols ...string) (*User, error) {
	...
	query := fmt.Sprintf(
		"select %s from `user` where `impart_wealth_id`=?", sel,
	)
...
func UserExists(ctx context.Context, exec boil.ContextExecutor, impartWealthID string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `user` where `impart_wealth_id`=? limit 1)"

...
var UserInstitutionRels = struct {
	ImpartWealth            string
	Institution             string
	UserInstitutionAccounts string
}{
	ImpartWealth:            "ImpartWealth",
	Institution:             "Institution",
	UserInstitutionAccounts: "UserInstitutionAccounts",
}
...
type userInstitutionR struct {
	...
	UserInstitutionAccounts UserInstitutionAccountSlice `boil:"UserInstitutionAccounts" json:"UserInstitutionAccounts" toml:"UserInstitutionAccounts" yaml:"UserInstitutionAccounts"`
}
...
// UserInstitutionAccounts retrieves all the user_institution_account's UserInstitutionAccounts with an executor.
func (o *UserInstitution) UserInstitutionAccounts(mods ...qm.QueryMod) userInstitutionAccountQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`user_institution_accounts`.`user_institutions_id`=?", o.UserInstitutionsID),
	)

	query := UserInstitutionAccounts(queryMods...)
	queries.SetFrom(query.Query, "`user_institution_accounts`")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"`user_institution_accounts`.*"})
	}
...
// LoadUserInstitutionAccounts allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userInstitutionL) LoadUserInstitutionAccounts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUserInstitution interface{}, mods queries.Applicator) error {
	var slice []*UserInstitution
	var object *UserInstitution
	...
	query := NewQuery(
		qm.From(`user_institution_accounts`),
		qm.WhereIn(`user_institution_accounts.user_institutions_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load user_institution_accounts")
	}

	var resultSlice []*UserInstitutionAccount
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice user_institution_accounts")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on user_institution_accounts")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for user_institution_accounts")
	}
	...
	if singular {
		object.R.UserInstitutionAccounts = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &userInstitutionAccountR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.UserInstitutionsID == foreign.UserInstitutionsID {
				local.R.UserInstitutionAccounts = append(local.R.UserInstitutionAccounts, foreign)
				if foreign.R == nil {
					foreign.R = &userInstitutionAccountR{}
				}
...
// AddUserInstitutionAccounts adds the given related objects to the existing relationships
// of the user_institution, optionally inserting them as new records.
// Appends related to o.R.UserInstitutionAccounts.
// Sets related.R.UserInstitution appropriately.
func (o *UserInstitution) AddUserInstitutionAccounts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserInstitutionAccount) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserInstitutionsID = o.UserInstitutionsID
			...
			values := []interface{}{o.UserInstitutionsID, rel.AccountID}
	...
	if o.R == nil {
		o.R = &userInstitutionR{
			UserInstitutionAccounts: related,
		}
	} else {
		o.R.UserInstitutionAccounts = append(o.R.UserInstitutionAccounts, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &userInstitutionAccountR{
				UserInstitution: o,
			}
		} else {

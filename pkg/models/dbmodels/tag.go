...
type Tag struct {
	TagID       uint   `boil:"tag_id" json:"tag_id" toml:"tag_id" yaml:"tag_id"`
	Name        string `boil:"name" json:"name" toml:"name" yaml:"name"`
	LongName    string `boil:"long_name" json:"long_name" toml:"long_name" yaml:"long_name"`
...
var TagWhere = struct {
	TagID       whereHelperuint
	Name        whereHelperstring
	LongName    whereHelperstring
	Description whereHelperstring
	SortOrder   whereHelperint
}{
	TagID:       whereHelperuint{field: "`tag`.`tag_id`"},
	Name:        whereHelperstring{field: "`tag`.`name`"},
	LongName:    whereHelperstring{field: "`tag`.`long_name`"},
	Description: whereHelperstring{field: "`tag`.`description`"},
	SortOrder:   whereHelperint{field: "`tag`.`sort_order`"},
}
...
var (
	tagAllColumns            = []string{"tag_id", "name", "long_name", "description", "sort_order"}
	tagColumnsWithoutDefault = []string{"name", "long_name", "description", "sort_order"}
	tagColumnsWithDefault    = []string{"tag_id"}
...
func (q tagQuery) One(ctx context.Context, exec boil.ContextExecutor) (*Tag, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "dbmodels: failed to execute a one query for tag")
	}
...
func (q tagQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: failed to count tag rows")
	}
...
func (q tagQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "dbmodels: failed to check if tag exists")
	}
...
func (o *Tag) Posts(mods ...qm.QueryMod) postQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("`post_tag` on `post`.`post_id` = `post_tag`.`post_id`"),
		qm.Where("`post_tag`.`tag_id`=?", o.TagID),
	)
...
func (tagL) LoadPosts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTag interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("`post`.post_id, `post`.hive_id, `post`.impart_wealth_id, `post`.pinned, `post`.created_ts, `post`.subject, `post`.content, `post`.last_comment_ts, `post`.comment_count, `post`.up_vote_count, `post`.down_vote_count, `a`.`tag_id`"),
		qm.From("`post`"),
	...
	for results.Next() {
		...
		err = results.Scan(&one.PostID, &one.HiveID, &one.ImpartWealthID, &one.Pinned, &one.CreatedTS, &one.Subject, &one.Content, &one.LastCommentTS, &one.CommentCount, &one.UpVoteCount, &one.DownVoteCount, &localJoinCol)
		if err != nil {
			return errors.Wrap(err, "failed to scan eager loaded results for post")
		}
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice post")
		}
...
func (o *Tag) AddPosts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Post) error {
	...
	for _, rel := range related {
		...
		values := []interface{}{o.TagID, rel.PostID}
...
func (o *Tag) RemovePosts(ctx context.Context, exec boil.ContextExecutor, related ...*Post) error {
	var err error
	query := fmt.Sprintf(
		"delete from `post_tag` where `tag_id` = ? and `post_id` in (%s)",
		strmangle.Placeholders(dialect.UseIndexPlaceholders, len(related), 2, 1),
	)
	values := []interface{}{o.TagID}
	for _, rel := range related {
		values = append(values, rel.PostID)
	}
...
func FindTag(ctx context.Context, exec boil.ContextExecutor, tagID uint, selectCols ...string) (*Tag, error) {
	tagObj := &Tag{}
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "dbmodels: unable to select from tag")
	}
...
func (o *Tag) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("dbmodels: no tag provided for insertion")
	}
	...
	if err != nil {
		return errors.Wrap(err, "dbmodels: unable to insert into tag")
	}
	...
	o.TagID = uint(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == tagMapping["tag_id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "dbmodels: unable to populate default values for tag")
	}
...
func (o *Tag) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("dbmodels: unable to update tag, could not build whitelist")
		}
	...
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: unable to update tag row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: failed to get rows affected by update for tag")
	}
...
func (q tagQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: unable to update all for tag")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: unable to retrieve rows affected for tag")
	}
...
func (o *Tag) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("dbmodels: no tag provided for upsert")
	}
	...
	if !cached {
		...
		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("dbmodels: unable to upsert tag, could not build update column list")
		}
	...
	if err != nil {
		return errors.Wrap(err, "dbmodels: unable to upsert for tag")
	}
	...
	if err != nil {
		return errors.Wrap(err, "dbmodels: unable to retrieve unique values for tag")
	}
...
func (o *Tag) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: unable to delete from tag")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: failed to get rows affected by delete for tag")
	}
...
func (q tagQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: unable to delete all from tag")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: failed to get rows affected by deleteall for tag")
	}
...
func TagExists(ctx context.Context, exec boil.ContextExecutor, tagID uint) (bool, error) {
	var exists bool
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "dbmodels: unable to check if tag exists")
	}

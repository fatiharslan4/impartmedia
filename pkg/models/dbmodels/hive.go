...
func (hiveL) LoadAdminImpartWealthUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeHive interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("`user`.impart_wealth_id, `user`.authentication_id, `user`.email, `user`.screen_name, `user`.created_at, `user`.updated_at, `user`.deleted_at, `user`.device_token, `user`.aws_sns_app_arn, `user`.admin, `user`.email_verified, `user`.blocked, `user`.feedback, `a`.`admin_hive_id`"),
		qm.From("`user`"),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ImpartWealthID, &one.AuthenticationID, &one.Email, &one.ScreenName, &one.CreatedAt, &one.UpdatedAt, &one.DeletedAt, &one.DeviceToken, &one.AwsSNSAppArn, &one.Admin, &one.EmailVerified, &one.Blocked, &one.Feedback, &localJoinCol)
		if err != nil {
...
func (hiveL) LoadMemberImpartWealthUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeHive interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("`user`.impart_wealth_id, `user`.authentication_id, `user`.email, `user`.screen_name, `user`.created_at, `user`.updated_at, `user`.deleted_at, `user`.device_token, `user`.aws_sns_app_arn, `user`.admin, `user`.email_verified, `user`.blocked, `user`.feedback, `a`.`member_hive_id`"),
		qm.From("`user`"),
